...
std::wstring select_document_file_name(std::optional<QString> root_dir) {
    if (DEFAULT_OPEN_FILE_PATH.size() == 0) {
        ...
        QString file_name = QFileDialog::getOpenFileName(nullptr, "Select Document", root_dir.value_or(""), "Documents (*.pdf *.epub *.cbz *.cbr *.cb7 *.cbt *.xps *.djv *.djvu *.fb2 *.png *.jpg *.jpeg *.jpe *.jfif *.bmp *.gif *.tif *.tiff *.webp)");
        return file_name.toStdWString();
    }
    else {

        QFileDialog fd = QFileDialog(nullptr, "Select Document", root_dir.value_or(""), "Documents (*.pdf *.epub *.cbz *.cbr *.cb7 *.cbt *.xps *.djv *.djvu *.fb2 *.png *.jpg *.jpeg *.jpe *.jfif *.bmp *.gif *.tif *.tiff *.webp)");
